...
const enableGnomeProxy = async (ip: string, port: string): Promise<void> => {
  ...
  try {
    await execPromise(
      `gsettings set org.gnome.system.proxy mode '${proxySettings.mode}'`,
    );
    log.info(`Proxy mode set to ${proxySettings.mode}`);

    await execPromise(
      `gsettings set org.gnome.system.proxy.socks host ${proxySettings.host}`,
    );
    log.info(`SOCKS host set to ${proxySettings.host}`);

    await execPromise(
      `gsettings set org.gnome.system.proxy.socks port ${proxySettings.port}`,
    );
    log.info(`SOCKS port set to ${proxySettings.port}`);

    log.info(
      `Proxy settings enabled for GNOME with SOCKS5 proxy: ${proxySettings.socks}`,
    );
  } catch (err) {
...
const enableKDEProxy = async (ip: string, port: string): Promise<void> => {
  ...
  try {
    ...
    log.info(
      `Proxy settings enabled for KDE with SOCKS5 proxy: ${proxySettings.socksProxy}`,
    );
  } catch (err) {
    ...
    return new Promise<void>(async (resolve, reject) => {
      try {
        ...
        if (method === 'psiphon') {
          ...
          log.info('🚀 ~ file: proxy.ts:65 ~ pacServeUrl:', pacServeUrl);
        }
